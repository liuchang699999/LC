...
REAL K(100,100),KE(12,12),AKE(12,12),X(50),Y(50),Z(50),AL(50),
 &EAI(6,20),PJ(20),PF(2,20),R(12,12),P(50),D_D(20),E,G,D_E, 	 !D_D为杆直径
 >DMAX,FE(12),D(50),ADE(12),DE(12),RT(12,12),F(6),AFE(12),AF(12)
INTEGER JE(3,50),JN(6,50),JPJ(20),JPF(3,20),M(12),JEAI(40)
OPEN (6,FILE='SFSAP.IN')
	OPEN (8,FILE='SFSAP.OUT')
CALL READ(NJ,NJJ,N,NE,NM,NPJ,NPF,JN,X,Y,Z,JE,JEAI,		 !读文件
 &JPJ,PJ,JPF,PF)
	 	DO IE=1,NE
D_D(IE)=0.01										 !初始化杆直径。
 END DO
...
WRITE(8,40)
40FORMAT(/'NO.E',5X,'DX',10X,'DY',10X,'DZ',
...
70FORMAT(I2,2X,6G12.4)
	END DO
WRITE(8,80)
80FORMAT(/'单元号',2X,'内力DMAX',2X,'临界应力D_E',2X,
 &'直径D_D',2X)
DO IE=1,NE
...
END If
	...
	SUBROUTINE READ(NJ,NJJ,N,NE,NM,NPJ,NPF,JN,X,Y,Z,JE, JEAI, !子程序读文件
 &JPJ,PJ,JPF,PF)
...
REAL R(12,12),X(50),Y(50),Z(50),barL,L2,L3,
...
LZY=ZX/L2
LZZ=XY/L2
S1=(1-LXX**2)*(X(K)-X(I))-LXX*LXY*(Y(K)-Y(I))
 &-LXX*LXZ*(Z(K)-Z(I))
S2=(1-LXY**2)*(Y(K)-Y(I))-LXY*LXX*(X(K)-X(I))
 &-LXY*LXZ*(Z(K)-Z(I))
S3=(1-LXZ**2)*(Z(K)-Z(I))-LXZ*LXX*(X(K)-X(I))
 &-LXZ*LXY*(Y(K)-Y(I))
...
SUBROUTINE PE(FE,IP,JPF,PF,AL)!子程序计算单元等效结点载荷
...
RETURN
END
SUBROUTINE MULV12(A,B,C)!子程序矩阵相乘
...
RETURN
END
SUBROUTINE MF(P,AFE,M)!子程序组集整体坐标系下载荷列阵
REAL P(50),AFE(12)
INTEGER M(12)
DO I=1,12
...
SUBROUTINE SLOV(AK,P,D,N)!子程序解方程求自由结点位移
...
RETURN
END
SUBROUTINE MADE(IE,JN,JE,D,ADE) !子程序求整体坐标系下单元杆端位移
REAL ADE(12),D(50)
